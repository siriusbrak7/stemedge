...
import { ORGANS, OrganInfo, DepthLevel } from '../../data/humanBodyData';
...
const SystemOrganInfoPanel: React.FC<Props> = ({ organId, depth, onDepthChange, onSaveNote }) => {
    const organ: OrganInfo | undefined = organId ? ORGANS[organId] : undefined;

    const handleSave = () => {
        if (organ) {
            onSaveNote(`${organ.name}: ${organ.description[depth]}`);
            alert("Note saved!");
        }
    };
    ...
    if (!organ) {
        return (
            <div className="h-full flex flex-col items-center justify-center text-center p-8 bg-slate-900/50 rounded-2xl border border-slate-800">
                ...
                <h3 className="text-xl font-bold text-white mb-2">Explore the Body</h3>
                <p className="text-slate-400">
                    Click on an organ in the diagram to reveal its function.
                </p>
            </div>
        );
    }
    ...
    return (
        <div className="flex flex-col h-full animate-fade-in-right">
            {/* Header */}
            <div className="mb-6">
                <h2 className="text-3xl font-bold text-white tracking-tight mb-2">{organ.name}</h2>
                <div className={`h-1 w-full rounded-full ${
                    organ.system === 'digestive' ? 'bg-amber-500' :
                    organ.system === 'respiratory' ? 'bg-cyan-500' : 'bg-red-500'
                }`} />
                <span className="text-xs font-bold uppercase tracking-wider text-slate-500 mt-2 block">
                    {organ.system} System
                </span>
            </div>
            ...
            <div className="flex-1 overflow-y-auto pr-2 mb-6">
                ...
                <div className="bg-slate-800/30 p-4 rounded-xl border border-slate-700/50 mb-4">
                    <div className="flex items-start gap-3">
                        ...
                        <div>
                            <p className="text-xs font-bold text-purple-300 uppercase mb-1">Did You Know?</p>
                            <p className="text-sm text-purple-100 italic">"{organ.funFact}"</p>
                        </div>
                    </div>
                </div>
            </div>
...
export default SystemOrganInfoPanel;
