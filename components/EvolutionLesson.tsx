...
import { ArrowLeft, ChevronLeft, ChevronRight, Zap, Menu, Dna } from 'lucide-react';
import { useLessonProgress } from '../hooks/useLessonProgress';
import { EVOLUTION_SLIDES, DepthLevel } from '../data/evolutionData';
import PepperedMothSim from './evolution/PepperedMothSim';
import FinchBeakSim from './evolution/FinchBeakSim';
import AntibioticResSim from './evolution/AntibioticResSim';
import EvolutionInfoPanel from './evolution/EvolutionInfoPanel';

interface Props {
    onBack: () => void;
}

const EvolutionLesson: React.FC<Props> = ({ onBack }) => {
    const { 
        currentSlide, 
        currentSlideIndex, 
        totalSlides, 
        ...
        completedSlides, 
        saveNote 
    } = useLessonProgress('evolution', EVOLUTION_SLIDES);
    ...
    const renderInteractiveArea = () => {
        switch (currentSlide.simType) {
            case 'moth':
                return <PepperedMothSim />;
            case 'finch':
                return <FinchBeakSim />;
            case 'antibiotic':
                return <AntibioticResSim />;
            case 'dna':
                return (
                    <div className="bg-slate-900 border border-slate-800 rounded-2xl p-8 flex flex-col items-center justify-center h-full text-center">
                        <div className="w-20 h-20 bg-blue-900/30 rounded-full flex items-center justify-center mb-6 animate-pulse border border-blue-500/30">
                            <Dna className="w-10 h-10 text-blue-400" />
                        </div>
                        <h3 className="text-xl font-bold text-white mb-4">Genetic Similarity</h3>
                        <div className="space-y-3 w-full max-w-sm text-left">
                            <div>
                                <div className="flex justify-between text-xs text-slate-400 mb-1"><span>Human vs Chimpanzee</span><span>98%</span></div>
                                <div className="w-full h-2 bg-slate-800 rounded-full"><div className="h-full bg-green-500 w-[98%] rounded-full"></div></div>
                            </div>
                            <div>
                                <div className="flex justify-between text-xs text-slate-400 mb-1"><span>Human vs Mouse</span><span>85%</span></div>
                                <div className="w-full h-2 bg-slate-800 rounded-full"><div className="h-full bg-yellow-500 w-[85%] rounded-full"></div></div>
                            </div>
                            <div>
                                <div className="flex justify-between text-xs text-slate-400 mb-1"><span>Human vs Banana</span><span>50%</span></div>
                                <div className="w-full h-2 bg-slate-800 rounded-full"><div className="h-full bg-red-500 w-[50%] rounded-full"></div></div>
                            </div>
                        </div>
                    </div>
                );
            default:
                return (
                    <div className="bg-slate-900 border border-slate-800 rounded-2xl p-8 h-full flex flex-col items-center justify-center text-center">
                        <div className="w-24 h-24 bg-gradient-to-br from-green-500 to-cyan-600 rounded-full flex items-center justify-center shadow-lg mb-6 animate-spin-slow">
                            <span className="text-4xl">🐢</span>
                        </div>
                        <h3 className="text-2xl font-bold text-white mb-2">{currentSlide.title}</h3>
                        <p className="text-slate-400 max-w-md">Explore the mechanisms of life's history using the interactive tools and information panel.</p>
                    </div>
                );
        }
    };
                ...
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h1 className="text-lg font-bold text-white flex items-center gap-2">
                            Evolution & Adaptation <span className="text-xs bg-green-900/50 text-green-300 px-2 py-0.5 rounded border border-green-500/30">Module 6</span>
                        </h1>
                    </div>
                </div>
