...
const AuthModal: React.FC<AuthModalProps> = ({ isOpen, onClose }) => {
  ...
  const handleForgotPassword = async (e: React.FormEvent) => {
    ...
    if (resetStep === 'ask') {
      ...
      const result = await authService.resetPassword(email, securityAnswer, password);
      if (result.error) {
        setError(result.error.message);
      } else {
        setMode('login');
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm">
      <div 
        ref={modalRef}
        className="relative w-full max-w-md bg-slate-900 border border-purple-500/30 rounded-2xl shadow-2xl max-h-[90vh] overflow-y-auto"
      >
        <button 
          onClick={onClose} 
        ...
        <div className="p-6">
          <h2 className="text-2xl font-bold text-white mb-2">
            ...
            {mode === 'signup' && 'Join StemEdge'}
            {mode === 'forgot' && 'Reset Password'}
          </h2>
          <p className="text-slate-400 text-sm mb-6">
            {mode === 'login' && 'Continue your cosmic learning journey'}
            {mode === 'signup' && 'Create your account to explore'}
            {mode === 'forgot' && 'We\'ll help you recover access'}
          </p>

          {error && (
            <div className="mb-4 p-3 bg-red-900/30 border border-red-500/50 rounded-lg flex items-start gap-2 text-red-200 text-sm">
          ...
          {mode === 'signup' && (
            <form onSubmit={handleSignUp} className="space-y-4">
              ...
              <button
                type="submit"
                disabled={loading}
                className="w-full py-2.5 bg-purple-600 hover:bg-purple-700 text-white font-bold rounded-lg"
              >
          ...
          {mode === 'forgot' && (
            <form onSubmit={handleForgotPassword} className="space-y-4">
              ...
              {resetStep === 'answer' && (
                <>
                  <p className="text-slate-300 text-sm">Security Question: {storedQuestion}</p>
                  <input
                    type="text"
                    value={securityAnswer}
                    onChange={(e) => setSecurityAnswer(e.target.value)}
              ...
              <button
                ...
                className="w-full py-2.5 bg-purple-600 hover:bg-purple-700 text-white font-bold rounded-lg"
              >
                {loading ? 'Processing...' : resetStep === 'ask' ? 'Find Account' : resetStep === 'answer' ? 'Verify Answer' : 'Reset Password'}
              </button>
            </form>
          )}
          ...
          <div className="mt-6 text-center space-x-2 text-sm">
            {mode === 'login' && (
              <>
                ...
                <button onClick={() => setMode('forgot')} className="text-purple-400 hover:text-purple-300">
                  Forgot password?
                </button>
              </>
            )}
            {mode !== 'login' && (
              <button onClick={() => { setMode('login'); setResetStep('ask'); }} className="text-purple-400 hover:text-purple-300">
                Back to login
              </button>
            )}
          </div>
